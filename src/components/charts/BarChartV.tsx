...
export default function BarChartV({ data, datasets, stacked = false, multiColor = false, yTickFormatter, tooltipFormatter }: Props) {
  const yFmt = yTickFormatter ?? ((v: number) => formatCLP(v, true))
  const tipFmt = tooltipFormatter ?? ((v: number, name: string) => [formatCLP(v), name])
  return (
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={data} margin={{ top: 4, right: 16, left: 0, bottom: 0 }} barCategoryGap="30%">
        ...
        <YAxis tick={{ fill: COLORS.text, fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={yFmt} width={60} />
        ...
        {datasets.map((ds, i) => {
          ...
          return (
            <Bar key={ds.key} dataKey={ds.key} name={ds.label} fill={color} radius={stacked ? [0, 0, 0, 0] : [6, 6, 0, 0]} stackId={stacked ? 'stack' : undefined} maxBarSize={48}>
              {multiColor && data.map((entry, idx) => (
                <Cell key={idx} fill={(entry.color as string | undefined) ?? PALETTE[idx % PALETTE.length]} />
              ))}
            </Bar>
          )
        })}
