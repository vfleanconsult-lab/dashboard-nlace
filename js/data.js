...
var state = {
rows: [], years: [], loaded: false, error: null, loadedAt: null
};
...
var cuenta = getCuenta(row).toLowerCase();
return (
desc.indexOf('retiro director') !== -1 ||
clas.indexOf('retiro director') !== -1 ||
cuenta.indexOf('retiro director') !== -1
);
}
...
var yearMap = {};
state.rows.forEach(function(r) { var y = getYear(r); if (y) yearMap[y] = true; });
state.years = Object.keys(yearMap).sort().reverse();
state.loaded = true;
...
if (/^\d{2}/\d{2}/\d{4}$/.test(s)) {
var p2 = s.split('/');
return new Date(parseInt(p2[2]), parseInt(p2[1]) - 1, parseInt(p2[0]));
}
var dt = new Date(s);
...
.forEach(function(r) {
var cliente = getDesc(r) || getCuenta(r);
var emi = parseDateCL(getFechaEmision(r)), pago = parseDateCL(getFechaPago(r));
if (!emi || !pago || pago < emi) return;
var diff = (pago - emi) / 86400000;
if (!map[cliente]) map[cliente] = { dias: 0, count: 0, monto: 0 };
map[cliente].dias += diff; map[cliente].count += 1; map[cliente].monto += getMonto(r);
});
...
var clas = getClasifCto(r) || getClasGasto(r) ||'Sin clasificar';
...
var clas = getClasGasto(r) || getClasifCto(r) ||'Sin clasificar';
map[clas] = (map[clas] || 0) + getMonto(r);
});
return map;
}
